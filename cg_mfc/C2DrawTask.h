...
{
	public:
	static void DoTask2D(CDC* pDC, CLayout* layout)
	{
		static C2DrawTask task;
		task.m_layout = layout;
		task.Task(pDC);
	}
	...
	{
		TaskPoint_Array(pDC);
		TaskLine_Triangle(pDC);
		TaskRectangle(pDC);
		TaskEllipse(pDC);
		TaskArc(pDC);
		TaskPie(pDC);
		TaskPie_BaGuaGraph(pDC);
		TaskPolygon(pDC);
		TaskPolygon_FiveStar(pDC);
		TaskFillSolidRect(pDC);
		TaskFillRect(pDC);
		TaskPath(pDC);
		TaskBezier_Curve(pDC);
		TaskBezier_MulSmoothCurve(pDC);
		//TaskBitmap(pDC);
		TaskText(pDC);
	}
...
private:
	...
	*/
	...
	//void TaskBitmap(CDC* pDC);


	void TaskText(CDC* pDC);
